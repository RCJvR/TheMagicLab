...
(function () {
  ...
  function blocksForEntry(entry) {
    ...
    entry.days.forEach(d => {
      if (e > s) {
        pieces.push({ day: d, start: s, end: e, entry });
      } else {
        pieces.push({ day: d, start: s, end: DAY_MIN, entry });
        pieces.push({ day: (d + 1) % 7, start: 0, end: e, entry });
      }
    });
    return pieces;
  }
  ...
  function renderGrid() {
    const allPieces = entries.flatMap(blocksForEntry);
    const byDay = Array.from({ length: 7 }, () => []);
    allPieces.forEach(p => byDay[p.day].push(p));
    ...
    const days = DAY_ABBR.map((name, d) => {
      const pieces = packLanes(byDay[d]);
      ...
      return `<div class="grid-day">
        <div class="grid-day-head${d === todayIdx ? ' today' : ''}">${name}</div>
        <div class="grid-day-col" style="height:${DAY_MIN * PX_PER_MIN}px;">${blocks}</div>
      </div>`;
    }).join('');

    document.getElementById('week-grid').innerHTML = `${gutter}<div class="grid-days">${days}</div>`;
  ...
  // ── Totals ───────────────────────────────────────────────────
  function weeklyMinutesByCategory() {
    const totals = {};
    Object.keys(CATEGORIES).forEach(c => { totals[c] = 0; });
    entries.forEach(e => { totals[e.category] += duration(e.start, e.end) * e.days.length; });
    return totals;
  }
  ...
  function renderTotals() {
    ...
    const totals = weeklyMinutesByCategory();
  ...
  function computeInsights() {
    ...
    const perDayHomework = Array(7).fill(0);
    entries.filter(e => e.category === 'homework').forEach(e => {
      const mins = duration(e.start, e.end);
      e.days.forEach(d => { perDayHomework[d] += mins; });
    });
    const heavyDay = perDayHomework.findIndex(m => m > 180);
    if (heavyDay >= 0) {
      insights.push({ level: 'warn', icon: '📚', text: `${DAY_FULL[heavyDay]} has more than 3 hours of homework/study blocked. Split long sessions into 25–45 minute focused blocks with short breaks — it works better than one long sitting.` });
    }
    ...
    if (!insights.length) {
      insights.push({ level: 'ok', icon: '✅', text: 'This looks like a balanced week — school, study, activities, rest and free time all have a place.' });
    }
    return insights;
  }
    ...
    box.innerHTML = sorted.map(e => {
      ...
      return `<div class="entry-row" data-id="${e.id}">
        ...
        <div class="entry-body">
          ...
          <div class="entry-meta">${esc(fmtDays(e.days))} · ${fmtTime(e.start)}–${fmtTime(e.end)}</div>
        </div>
  ...
  function wireForm() {
    ...
    const daysBox = document.getElementById('af-days');
    daysBox.innerHTML = DAY_ABBR.map((name, d) => `<button type="button" class="day-btn" data-day="${d}">${name}</button>`).join('');
    daysBox.querySelectorAll('.day-btn').forEach(btn => {
    ...
    document.querySelectorAll('.day-quick [data-preset]').forEach(btn => {
      btn.addEventListener('click', () => {
        const preset = btn.dataset.preset;
        if (preset === 'weekdays') selectedDays = new Set([0, 1, 2, 3, 4]);
        else if (preset === 'weekend') selectedDays = new Set([5, 6]);
        else if (preset === 'everyday') selectedDays = new Set([0, 1, 2, 3, 4, 5, 6]);
        else selectedDays = new Set();
        daysBox.querySelectorAll('.day-btn').forEach(b => b.classList.toggle('active', selectedDays.has(Number(b.dataset.day))));
      });
    });
    ...
    document.getElementById('af-submit').addEventListener('click', () => {
      ...
      if (!selectedDays.size) { msg.textContent = 'Pick at least one day.'; msg.className = 'msg err'; return; }
      ...
      addEntry({ category, title: title || null, days: [...selectedDays], start, end });
      msg.textContent = '';
